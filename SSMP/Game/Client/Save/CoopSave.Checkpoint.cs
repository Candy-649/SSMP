...
internal partial class CoopSave {
    ...
    /// <summary>
    /// The partner's game saw a boss fight start with both players, so it gets a checkpoint here too if the local
    /// player is in its scene.
    /// </summary>
    ...
    /// <summary>
    /// Keeps the boss checkpoint of the loaded save up to date every frame: ends it when its boss was beaten, takes the
    /// local player out of a fight that their partner dropped out of, saves the game for a fight that started, and
    /// takes the local player back to the door after loading a save whose fight lasted.
    /// </summary>
    private void UpdateCheckpoint(HeroController hero, CoopSaveMarker marker, ClientPlayerData? partner) {
        ...
        // The player goes back once they got up from the bench that the save loaded at
        ...
        _loadedWithCheckpoint = false;
        ReturnToCheckpoint(hero, marker);
    }
    ...
    /// </summary>
    private void LeaveBossFight(HeroController hero, CoopSaveMarker marker, string scene) {
        // What the intro of the fight set for the hero, like not being able to pause, doesn't come along
        var playerData = PlayerData.instance;
        ...
        if (door != null && !string.IsNullOrEmpty(door.targetScene) && door.targetScene != scene &&
            ...
            Logger.Info($"The partner dropped out of the boss fight, leaving through door '{gate}'");
            MoveTo(hero, new global::GameManager.SceneLoadInfo {
        ...
        Logger.Info($"Door '{gate}' of the boss fight doesn't lead out, coming back in through it");
        MoveTo(hero, new global::GameManager.SceneLoadInfo {
            SceneName = scene,
            EntryGateName = gate,
    ...
    /// </summary>
    private void ReturnToCheckpoint(HeroController hero, CoopSaveMarker marker) {
    ...
    /// </summary>
    private void MoveTo(HeroController hero, global::GameManager.SceneLoadInfo info) {
        _checkpointMoveTime = Time.unscaledTime;
        hero.RecordLeaveSceneCState();
        global::GameManager.instance.BeginSceneTransition(info);
    }
