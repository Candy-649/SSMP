...
internal partial class GamePatcher {
    ...
    public void RegisterHooks() {
        ...
        // Remote players' attacks must not knock back the local player
    ...
    /// <summary>
    /// Creates a hook for the method with the given name on the given type and stores it, so it is disposed when the
    ...
    private void AddHook(Type type, string methodName, Delegate detour) {
        if (TryCreateHook(type, methodName, detour) is { } hook) {
            _hooks.Add(hook);
        }
    }
